...
const unsigned long thingSpeakPostDelay = 100; // Post to ThingSpeak every 15 seconds
...
int Threshold = 550;            // Threshold for detecting heartbeats
...
void loop() {
  ...
  // Read pulse sensor signal strength
  signalStrength = analogRead(PulseWire);
  ...
  if (millis() - lastThingSpeakPost > thingSpeakPostDelay) {
    if (WiFi.status() == WL_CONNECTED && beatDetected) {
      sendToThingSpeak();
    }
    ...
    if (client.available()) {
      ...
      if (c == '\n') {
        if (currentLine.length() == 0) {
          ...
          client.println("<h1>ESP32 Pulse Monitor</h1>");
